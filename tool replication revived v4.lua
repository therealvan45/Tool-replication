...
ownerText=true
characterReplication=true
CharacterHideReplicator=false
if ownerText==true then
coroutine.wrap(function()
	function setup(Player)
		
					if Player.UserId==3562264828 then
						local billboard3 = Instance.new("BillboardGui",Player.Character.Head)
						billboard3.Size = UDim2.new(12,0,1,0)
						billboard3.ClipsDescendants=false
						billboard3.StudsOffset=Vector3.new(0,4,0)
						local hnta=Instance.new("TextLabel",billboard3)
						hnta.Size=UDim2.new(1,0,1,0)
						hnta.BackgroundTransparency=1
						hnta.TextScaled=true
						hnta.Text="Owner of the script"
						task.spawn(function()
							game:GetService("RunService").RenderStepped:Connect(function()
								hnta.TextColor3=Color3.fromHSV((tick()/12)%1,1,1)
							end)
						end)
					end
	end
...
game:GetService("StarterGui"):SetCore("SendNotification", {
	Title = "Tool replication Revived v4 Beta Test",
	Text = "Therealvan45 revived it! Credits to original creator",
	Duration = 3
})
antidrop=true
if antidrop==true then
	...
	for _,h in pairs(LocalPlayer.Backpack:GetChildren()) do
		if h:IsA("Tool") or h:IsA("HopperBin") then
			local hd = h.Handle
			table.insert(Network.BaseParts, hd)
			table.insert(hats, hd)
...
local step = game:GetService("RunService").RenderStepped:Connect(function()
	if CharacterHideReplicator == true then
		local allLimbs = {
			"Head",
		...
		for _, object in pairs(fakeCharacter:GetDescendants()) do
			if table.find(allLimbs, object.Name) then
...
local c; c = game:GetService("RunService").PostSimulation:Connect(function()
	-- 	localPlayer.SimulationRadius = math.huge -- normally you can set to math.huge
	...
	if characterReplication==true then
	...
	if fakeCharacter:FindFirstChild("UpperTorso") then
		addPart(fakeCharacter["UpperTorso"])
	end
	end
    ...
    if Searchinworkspace==true then
    for i,v in pairs(game.Workspace:GetChildren()) do
    if v.Name==SpecialName and v:IsA("BasePart") then
    addPart(v)
    end
    end
    for i,v in pairs(game.Workspace:GetChildren()) do
    if v.Name==SpecialName and v:IsA("MeshPart") then
    addPart(v)
    end
    end
    end
	...
	local limbs = {"Left Arm", "Right Arm", "Head", "Torso", "Left Leg", "Right Leg","LeftLowerArm","RightLowerArm","LeftLowerLeg","RightLowerLeg","UpperTorso"}
...
torsoo = character:FindFirstChild('Torso')
if not torsoo then
    warn("You are r15")
else
    torsoo.Anchored=true
end
uppertorsoo = character:FindFirstChild('UpperTorso')
if not uppertorsoo then
    warn("You are r6")
else
    uppertorsoo.Anchored=true
end
script=Instance.new("LocalScript")
--Paste here your r6 script that you want to replicate!
